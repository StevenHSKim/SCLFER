...
# Grad Cam 생성용 MobileFaceNet 클래스
class MobileFaceNet(Module):
    def __init__(self, input_size, embedding_size=512, output_name="GDC"):
        super(MobileFaceNet, self).__init__()
        assert output_name in ["GNAP", 'GDC']
    ...
    def forward(self, x):
        ...
        #     raise ValueError("conv_features is None in MobileFaceNet forward pass.")
        # print("Conv_6_sep output shape:", conv_features.shape)

        # conv_features.requires_grad_()  # Grad 활성화

        logits = self.output_layer(conv_features)  # [B, num_classes]
        # if logits is None:
        #     raise ValueError("logits is None in MobileFaceNet forward pass.")
        # print("Logits shape:", logits.shape)

        return logits, conv_features


    def get_logits_for_grad_cam(self, x):
        """Return logits for Grad-CAM, ensuring scalar outputs for specific classes."""
        logits, _ = self.forward(x)
        return logits


# model = MobileFaceNet([112, 112],136)
# checkpoint = torch.load('pretrain/mobilefacenet_model_best.pth.tar', map_location=lambda storage, loc: storage)
