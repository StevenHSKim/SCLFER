...
def parse_args():
    ...
    parser.add_argument('--raf_path', type=str, default='C:/Users/steve/OneDrive/Desktop/SCLFER/datasets/raf-basic/', help='Raf-DB dataset path.')
    ...
    # Center Loss related parameters
    ...
    parser.add_argument('--checkpoint_dir', type=str, default='C:/Users/steve/OneDrive/Desktop/SCLFER/checkpoints', help='Directory to save checkpoints')
...
class CenterLoss(nn.Module):
    def __init__(self, device, num_classes=7, feat_dim=512):
    ...
    def forward(self, x, labels):
        ...
        # 각 클래스별 중심과 특징 벡터 간의 유클리디안 거리 계산
        centers_batch = self.centers.index_select(0, labels)
        diff = x - centers_batch
        loss = 1/2.0 * torch.sum(diff * diff) / batch_size
        
        return loss

def train(train_loader, val_loader, model, criterion_cls, criterion_center, optimizer, optimizer_center, scheduler, 
         device, epochs, patience, iteration, center_loss_weight, dataset_name, checkpoint_dir, class_labels):
    best_loss = float('inf')
    best_acc = 0
            ...
            if acc > best_acc:
                ...
                if acc > 0.60:
                    best_checkpoint_path = save_best_model(
                        ...
                        optimizer=optimizer,
                        optimizer_center=optimizer_center,  # Center Loss 옵티마이저 추가
                        iteration=iteration,
...
def test(test_loader, model, checkpoint_path, criterion_cls, criterion_center, device, center_loss_weight,
        ...
        with torch.no_grad():
            ...
            try:
                ...
                test_cm_filename = os.path.join(
                    checkpoint_dir,
                    f"centerloss_mfnonlychannel_{dataset_name}_iter{iteration+1}_epoch{epoch}_test_cm.png"
                )
...
def run_train_test():
    ...
    for iteration, (train_val_indices, test_indices) in enumerate(splits):
        ...
        model = SCLFER(num_head=1)  # num_head를 1로 고정
        model.to(device)
        # 모델 파라미터 수 출력
        ...
        criterion_center = CenterLoss(device, num_classes=7, feat_dim=512)  # RAF-DB에는 7개 감정 클래스가 있음
        ...
        # Center Loss 파라미터를 위한 별도의 옵티마이저 (일반적으로 SGD 사용)
        optimizer_center = torch.optim.SGD(criterion_center.parameters(), lr=args.center_lr)
        ...
        try:
            best_checkpoint_path, best_acc, best_val_true, best_val_pred, best_val_balanced_acc = train(
                ...
                criterion_center=criterion_center,  # Center Loss로 변경
                optimizer=optimizer, 
                optimizer_center=optimizer_center,  # Center Loss 옵티마이저 추가
                scheduler=scheduler, 
                device=device, 
                ...
                iteration=iteration,
                center_loss_weight=args.center_loss_weight,  # Center Loss 가중치 추가
    ...
    if results:
        ...
        results_df.to_csv('DAN_MobileNetFace_CenterLoss_test_results.csv', index=False)  # 결과 파일명 변경

        print("/nBest Accuracies over all iterations:")
        ...
        if all_accuracies:
            ...
            print(f"/nMean Test Accuracy over {len(all_accuracies)} iterations: {mean_accuracy:.4f} ± {std_accuracy:.4f}")
        else:
            print("/nNo results were collected from any iteration.")
